...
If the advection or diffusivity terms are nonlinear functions of \(u\),
we can weight these terms as
\begin{align*}
    v\left( u\left( x_i,\: t \right) \right) & = v\left( u_i \right)                                     &  & \text{(current node)} \\
    v\left( u\left( w_i,\: t \right) \right) & = \left( 1 - \sigma \right) v\left( u_{i-1} \right) + \sigma v\left( u_i \right) &  & \text{(west node)}    \\
    v\left( u\left( e_i,\: t \right) \right) & = \left( 1 - \sigma \right) v\left( u_i \right) + \sigma v\left( u_{i+1} \right) &  & \text{(east node)}
\end{align*}
for the advection, and
\begin{align*}
    D\left( u\left( x_i,\: t \right) \right) & = D\left( u_i \right)                                     &  & \text{(current node)} \\
    D\left( u\left( w_i,\: t \right) \right) & = \left( 1 - \sigma \right) D\left( u_{i-1} \right) + \sigma D\left( u_i \right) &  & \text{(west node)}    \\
    D\left( u\left( e_i,\: t \right) \right) & = \left( 1 - \sigma \right) D\left( u_i \right) + \sigma D\left( u_{i+1} \right) &  & \text{(east node)}
\end{align*}
for the diffusivity.
